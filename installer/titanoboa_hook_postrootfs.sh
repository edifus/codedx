...
echo "CodeDX release $VERSION_ID ($VERSION_CODENAME)" >/etc/system-release
...
e2label "\$xboot_dev" "codedx_xboot"
%end
...
_APP_NAME="CodeDX Bootloader Restoring Tool"
DRY_RUN=${DRY_RUN:-0}
MNT=/tmp/mnt
...
DISK_PATH=$(lsblk -d -n -o NAME,SIZE,MODEL | while read -r name size model; do
    ...
    --text="Double-click the disk where you installed CodeDX:" --width=500 --height=300 \
    --column="Device" \
    --column="Size" \
    --column="Model" \
    --print-column=1) || {
...
cat >/usr/share/applications/codedx_bootloader_restoring_tool.desktop <<'EOF'
[Desktop Entry]
Type=Application
Icon=tools-wizard-symbolic
Name=CodeDX Bootloader Restoring Tool BETA
...
if [[ $desktop_env == gnome ]]; then
    sed -i 's| Fedora| CodeDX|' /usr/share/anaconda/gnome/fedora-welcome || :
